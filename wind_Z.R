...
png(paste("wind_Z_",fine,"_alpi.png",sep=""), width = width, height = height)
...
grafico_lineare <- levelplot (zvar ~ xvar * yvar, data = gdr,
                   ...
                   at= unique(c(seq(0, 20, length=21))),
                   main="VELOCITA VENTO Lombardia da Rete ARPA",
                   col.regions = colorRampPalette(c('blue','red','yellow','green','black')),
...
png(paste("wind_Z_",fine,"_pianura.png",sep=""), width = width, height = height)
...
myPanel_log <- function(x=xvar, y=yvar, z=zvar, ..., subscripts=subscripts) {
        ...
        grafico_logaritmico <- levelplot (zvar ~ xvar * yvar, data = gdr,
         ...
         at= unique(c(seq(0, 20, length=21))),
         main="VELOCITA VENTO Lombardia da Rete ARPA - focus Pianura",
         col.regions = colorRampPalette(c('blue','red','yellow','green','black')),
