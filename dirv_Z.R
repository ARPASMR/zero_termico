...
y<-unique(quota)
  ...
  pp<-which(prec < 3)   # vento > 4 m/s
  gdr$zvar[pp] <- -999              # trattino su mancanti

### GRAFICO LINEARE PER FOCUS ALPI

png(paste("dirv_Z_",fine,"_alpi.png",sep=""), width = width, height = height)
...
myPanel_lin <- function(x=xvar, y=yvar, z=zvar, ..., subscripts=subscripts) {
     ...
     #           pp<-which(prec > 4)   # vento > 4 m/s
     #           z[pp] <- -999              # trattino su mancanti
...
png(paste("dirv_Z_",fine,"_pianura.png",sep=""), width = width, height = height)
